...
//                System.runFinalization();
        ...
        mOnCompressCompletedListener = new OnCompressCompletedListener() {
            @Override
            public void onCompressCompleted(CompressResult compressResult) {
                ...
                for (String success: compressResult.mSuccessPaths){
                    Log.e(">>>>>", "success ->: "+ success);
                }
                for (String fail: compressResult.mExceptionPaths){
                    Log.e(">>>>>", "fail ->: "+ fail);
                }
            }
        };
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && requestCode == PhotoPicker.REQUEST_CODE) {
            if (data != null) {
                ...
                Glide.with(MainActivity.this).load(photos.get(0)).into(mImageView);
                //默认异步使用
                if (mBiscuit==null) {
                    mBiscuit =
                Biscuit.with(this)
